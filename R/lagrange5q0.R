...
#' @param region  Character: 'Sub-Saharan regions', 'Eastern Africa', 'Middle Africa', 'Western Africa', or 'South Asia' (only for Model B).
...
#'
#'
#'To use coefficients for sub-Saharan Africa and south Asia, specify 'B' for the parameter 'model'. In the case scenario of having a single mortality input and no information on the value of k, the user can specify the parameter 'region' ('Sub-Saharan regions', 'Eastern Africa', 'Middle Africa', 'Western Africa', or 'South Asia') in order to benefit from a regional prior of k instead of assuming k = 0 (see example 6 below)
#'
#'
#' @export
#'
...
lagrange5q0 <- function(data,k, model, region){
  ...
  if(model %in%   c("B")){
    data(coef)
    pred <- subset(coef, sex == unique(data$sex) & model == 'B')
  }else{
  data(coef)
    pred <- subset(coef, sex == unique(data$sex) & model == 'A')
  }
  ...
  if(nrow(data) > 1){
    ...
    if(missing(k) == F){
      ...
      if(model == 'B' & missing(region) == F){
        if(F %in% (region   %in%   c("Sub-Saharan regions",
                                     "Eastern Africa",
                                     "Middle Africa",
                                     "Western Africa",
                                     "South Asia")))       stop('Wrong value for "region". Choose "Sub-Saharan regions", "Eastern Africa", "Middle Africa", "Western Africa", or "South Asia".')
      }
