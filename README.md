...
[Guillot M., J. Romero Prieto, A. Verhulst, P. Gerland. 2022. Modeling
Age Patterns of Under-5 Mortality: Results From a Log-Quadratic Model
Applied to High-Quality Vital Registration Data.
*Demography*](https://doi.org/10.1215/00703370-9709538).

[Romero-Prieto J. , A. Verhulst, and M. Guillot. 2024. Estimating 1a0
and 4a1 in a Life Table: A Model Approach Based on Newly Collected Data.
*Demography*](https://doi.org/10.1215/00703370-11330227).

[Verhulst A., J. Romero-Prieto, and M. Guillot. 2025. Modeling age
patterns of under-5 mortality: A comprehensive model life table approach
for low- and middle-income countries. *Population Center Working Paper
(PSC/PARC)*](https://doi.org/20.500.14332/61535).
...
age. A variety of mortality inputs between 0 and 5 years can be used to
predict a series of 22 cumulative probabilities of dying (q(x)) and
mortality rates (nMx) for the first 5 years of life.

Model A published by Guillot et al. in 2022 is based on historical data
from high-income countries compiled in the *Under-5 Mortality Database*
(U5MD). R code to replicate the coefficients of model A are available on
the website of the U5MD:
<https://web.sas.upenn.edu/global-age-patterns-under-five-mortality/>.

For model A, the package also computes the average number of years lived
in the age intervals 0-1, 1-4 and 0-5 (i.e., 1a0, 4a1, and 5a0) based on
the results of Romero-Prieto et al. (2024).
...
To use the coefficients for sub-Saharan Africa and south Asia, specify
the ‘B’ model in the function **lagrange5q0** (see example 6 below).
...
Model A is based on a set of vital records from high-income countries.
Predictions based on values of q(5y) above 0.150 and on values of k
below -1.1 or above +1.5 are extrapolations. In Model B based on DHS
data from sub-Saharan Africa and south Asia, the corresponding values
are 0. 330 for q(5y) and -1.8 and +1.2 for k. A warning message will be
displayed if these limits are exceeded. Beyond these limits, it also
might be impossible to converge to a solution. An error message will be
